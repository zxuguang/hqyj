...
import com.zxg.reggie.common.BaseContext;
...
public class AddressBookController {
    ...
    public R<AddressBook> getById(@PathVariable Long id){
        ...
        if ((byId==null)){
            throw new CustomException("桌号不存在");
        }
    ...
    public R<String> updateAdd(@RequestBody AddressBook addressBook) {
        if (addressBook == null) {
            throw new CustomException("桌号信息不存在，请刷新重试");
        }
        addressBookService.updateById(addressBook);
        return R.success("桌号修改成功");
    }
    @GetMapping("/list")
    ...
    public R<String> deleteAdd(@RequestParam("ids") Long id) {
        if (id == null) {
            throw new CustomException("桌号不存在，请刷新重试");
        }
